...
HRESULT 
CRTCAVSession::DeliverSessionState(RTC_SESSION_STATE enState)
{
	...
	if(enState == RTCSS_CONNECTED) {
		...
		if ((lMediaType & RTCMT_T120_SENDRECV) == 0) {
			EnableMenuItem(hMenu, ID_VIEW_APPSHARING, MF_GRAYED);
			EnableMenuItem(hMenu, ID_VIEW_WHITEBOARD, MF_GRAYED);
		} else {
			EnableMenuItem(hMenu, ID_VIEW_APPSHARING, MF_ENABLED);		
			EnableMenuItem(hMenu, ID_VIEW_WHITEBOARD, MF_ENABLED);		
		}
...
void
CRTCAVSession::StartMeasuringQuality()
{
...
void
CRTCAVSession::StopMeasuringQuality()
{
...
/////////////////////////////////////////////
//
// CRTCAVSession::ShowAppSharing
// 

HRESULT CRTCAVSession::ShowAppSharing()
{
	IRTCClient * pClient = NULL;
    HRESULT hr;
	VARIANT_BOOL fAppletRunning = FALSE;
...
////////////
	...
	hr = pClient->get_IsT120AppletRunning(RTCTA_APPSHARING, &fAppletRunning);
    ...
    if (!fAppletRunning)
    {
        ...
        hr = pClient->StartT120Applet(RTCTA_APPSHARING);

        if (FAILED(hr))
        {
			// Start T120 Applet failed
            SAFE_RELEASE(pClient);
            return hr;
        }
    ...
    {
		// there is nothing to do here, we do not support removing applet	
    }
...
/////////////////////////////////////////////
//
// CRTCAVSession::ShowWhiteBoard
// 

HRESULT CRTCAVSession::ShowWhiteBoard()
{
	IRTCClient * pClient = NULL;
    HRESULT hr;
	VARIANT_BOOL fAppletRunning = FALSE;
...
////////////
	...
	hr = pClient->get_IsT120AppletRunning(RTCTA_WHITEBOARD, &fAppletRunning);
    ...
    if (!fAppletRunning)
    {
        ...
        hr = pClient->StartT120Applet(RTCTA_WHITEBOARD);
...
// Begin RTC Functionality Code
//
// This code invokes methods that do RTC stuff, based on commands.
//
////////////
	...
	case ID_VIEW_APPSHARING:
        ShowAppSharing();
        break;

	case ID_VIEW_WHITEBOARD:
        ShowWhiteBoard();
        break;
...
HRESULT CRTCAVSession::DeliverMedia(long lMediaType, RTC_MEDIA_EVENT_TYPE enType, RTC_MEDIA_EVENT_REASON enReason)
{
	...
	case RTCMT_T120_SENDRECV:
		...
		// just return, dont do any video stuff.
		return S_OK;
