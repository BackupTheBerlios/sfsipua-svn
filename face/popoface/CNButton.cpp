...
//			itemRect.DeflateRect(1, 1);		// @@
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		if (szText.cx <= captionRect.Width()) {
			// Center text
			CRect centerRect = captionRect;
			pDC->DrawText(sTitle, -1, captionRect, Format );
...
HBITMAP CNButton::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CNButton::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CNButton::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CNButton::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC		hdcBmpMem	= NULL;
...
//
DWORD CNButton::SetIcon(int nIconIn, int nIconOut)
{
	HICON		hIconIn		= NULL;
	HICON		hIconOut	= NULL;
...
//			Failed loading the specified resource.
//
DWORD CNButton::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CNButton::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD CNButton::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int	nRetValue;
	BITMAP	csBitmapSize;
...
//				By default, CNButton buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CNButton::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CNButton buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CNButton::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//			Function executed successfully.
//
DWORD CNButton::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CNButton::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CNButton::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CNButton::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//			Function executed successfully.
//
DWORD CNButton::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CNButton::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CNButton::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CNButton::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void CNButton::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CNButton::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CNButton::ActivateTooltip(BOOL bActivate)
{
...
//				If TRUE the control will be repainted.
//
void CNButton::DrawTransparent(BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CNButton::SetURL(LPCTSTR lpszURL)
{
...
//			Failed loading the specified resource.
//
DWORD CNButton::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
// This is a virtual function that can be rewritten in CNButton-derived classes
...
//			Function executed successfully.
//
DWORD CNButton::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CNButton::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
...
void CNButton::setText(UINT nID)
{
	CString str;
	str.LoadString(nID);
	SetWindowText(str);
